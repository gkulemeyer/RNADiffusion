...
EXPERIMENT_NAME = "ArchiveII_test_ckpt"


# ------------------------------------------------------------
# helpers
# ------------------------------------------------------------
def build_job_dir(experiment_name, partition, timestep, base_dim, seed):
    return Path("logs") / experiment_name / partition / f"t{timestep}" / f"bd{base_dim}" / f"bs{BATCH_SIZE}_acc{ACCUMULATE_GRAD_BATCHES}" / f"seed{seed}"
...
def build_run_config(base_config, experiment_name, partition, timestep, epochs, base_dim):
    ...
    job_dir = build_job_dir(experiment_name, partition, timestep, base_dim, seed)
